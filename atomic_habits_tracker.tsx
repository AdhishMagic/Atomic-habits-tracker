...
try {
  appId = typeof __app_id !== 'undefined' ? __app_id : 'atomic-habits-tracker';
...
const DEFAULT_HABITS = [
  'Wake Up On Time', 'Journaling', 'Exercise', 'Watch SQL Videos', 'Breakfast', 
  'Apply for Jobs', 'Attend Interviews', 'Master DSA', 'LeetCode Coding', 
  'Unstop POTD', 'Lunch', 'List Companies', 'Aptitude Learning', 
  'Work on Projects', 'Go for a Walk', 'Learn Current Stock Market', 
  'Dinner', 'Text Her', 'Sleep On Time'
];
...
const YEARS = Array.from({ length: 25 }, (_, i) => 2026 + i); // 2026 to 2050

// Access PIN securely checks environment variables first
let envPin = '1812';
try {
  if (typeof process !== 'undefined' && process.env) {
    envPin = process.env.REACT_APP_PIN || process.env.VITE_APP_PIN || process.env.NEXT_PUBLIC_APP_PIN || '1812';
  }
} catch (error) {
  // safe fallback
}
const EDIT_PIN = envPin;

export default function App() {
  // Access Control State
  const [accessMode, setAccessMode] = useState(() => sessionStorage.getItem('atomicHabitsAccess') || null);
  ...
  // Modal States
  const [habitModal, setHabitModal] = useState({ isOpen: false, mode: 'add', habitName: '', inputValue: '' });
  const [confirmModal, setConfirmModal] = useState({ isOpen: false, action: null, text: '' });

  // 1. Initial Local Storage Load (Fast UI)
  useEffect(() => {
    const savedData = localStorage.getItem('atomicHabitsData');
    const savedHabits = localStorage.getItem('atomicHabitsList');
  ...
  // 3. Firebase Real-time Listener (Cross-device sync)
  useEffect(() => {
    if (!user || !db) return;
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (cloudData.data) {
            ...
            localStorage.setItem('atomicHabitsData', JSON.stringify(cloudData.data));
        }
        if (cloudData.habits) {
            setHabits(cloudData.habits);
            localStorage.setItem('atomicHabitsList', JSON.stringify(cloudData.habits));
        }
      }
  ...
  const pushToCloud = async (newData, newHabits) => {
    if (!user || !db) return;
    setSyncStatus('syncing');
    try {
      const docRef = doc(db, 'artifacts', appId, 'users', user.uid, 'trackerData', 'main');
  ...
  const toggleHabit = (dateStr, habit) => {
    setData(prev => {
      ...
      localStorage.setItem('atomicHabitsData', JSON.stringify(newData));
  ...
  const openEditHabitModal = (habit) => setHabitModal({ isOpen: true, mode: 'edit', habitName: habit, inputValue: habit });
  ...
  const handleModalSave = () => {
    ...
    if (habitModal.mode === 'add') {
       if (!habits.includes(newName)) {
          ...
          setHabits(newHabits);
          localStorage.setItem('atomicHabitsList', JSON.stringify(newHabits));
  ...
  const renderSettings = () => {
    const handleAddHabit = () => {
      ...
      if (newHabit && !habits.includes(newHabit)) {
          ...
          localStorage.setItem('atomicHabitsList', JSON.stringify(newHabitsList));
          pushToCloud(data, newHabitsList);
      }
    };
    
    const handleRemoveHabit = (h) => {
      if(confirm(`Remove habit "${h}"? Data will be hidden but kept in the background.`)) {
        const newHabitsList = habits.filter(habit => habit !== h);
        setHabits(newHabitsList);
        localStorage.setItem('atomicHabitsList', JSON.stringify(newHabitsList));
        pushToCloud(data, newHabitsList);
      }
    ...
    <div className="flex flex-col md:flex-row h-screen bg-gray-100 font-sans selection:bg-blue-100 overflow-hidden relative">
      ...
      `}>
        <div className="p-6 hidden md:block">
          ...
          <div className="flex items-center justify-between mt-2">
             ...
             {syncStatus === 'synced' ? <Cloud size={14} className="text-green-500" title="Synced to Cloud"/> : <CloudOff size={14} className="text-gray-500"/>}
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-gray-50/50 relative h-[calc(100vh-60px)] md:h-screen w-full">
        <style dangerouslySetInnerHTML={{__html: `
          ...
          /* Custom scrollbar for table (Excel feel) */
          .excel-scroll::-webkit-scrollbar { width: 10px; height: 10px; }
          .excel-scroll::-webkit-scrollbar-track { background: #f8fafc; border-radius: 4px; border: 1px solid #e2e8f0; }
